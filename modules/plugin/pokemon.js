...
function gacha(context, replyFunc) {
    ...
    mongodb(db_path, {
        ...
        let user_profile = await coll_pkm_stg.findOne({
        ...
        if (user_profile == undefined) {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                choice_list = (await coll_loc.findOne({
                    location: location
                }, {
                    projection: {
                        _id: 0,
            ...
            await coll_pkm_stg.insertOne({
                player_id: context.user_id,
                player_name: context.nickname,
                list: pokemon_list,
                storage: [],
                last_win: 0,
                last_capture: time,
            ...
            let info = await coll_loc.findOne({
                location: location
            }, {
                projection: {
                    _id: 0,
...
function fight(context, replyFunc) {
    ...
    mongodb(db_path, {
        ...
        let player_data_a = await coll_pkm_stg.findOne({
            player_id: player_a
        }, {
            projection: {
                _id: 0
        ...
        let player_data_b = await coll_pkm_stg.findOne({
            player_id: player_b
        }, {
            projection: {
                _id: 0
        ...
        if (player_data_a == undefined) text = "你必须先捕捉才能对战";
            ...
            if (time - player_data_a.last_fight < 3600000) text = `还需要${Math.ceil((3600000 - time + player_data_a.last_fight)/60000)}分钟才能再次对战`;
                ...
                if (player_data_a.last_win == player_b || player_data_b.last_win == player_a) {
                    ...
                    for (let i = 0; i < 6; i++) {
                        pkm_a = await coll_pokedex.findOne({
                            name: pokemon_list_a[player_a_box[i]]
                        });
                        pkm_b = await coll_pokedex.findOne({
                            name: pokemon_list_b[player_b_box[i]]
                        });
...
function checkList(context, replyFunc) {
    ...
    mongodb(db_path, {
        ...
        let coll_pkm_stg = mongo.db('bot').collection('pokemon_storage');
        let profile = await coll_pkm_stg.findOne({
            player_id: player_a
        }, {
            projection: {
                _id: 0
...
function changeList(context, replyFunc) {
    ...
    mongodb(db_path, {
        ...
        let coll_pkm_stg = mongo.db('bot').collection('pokemon_storage');
        let player_data = await coll_pkm_stg.findOne({
            player_id: player_id
        }, {
            projection: {
                _id: 0
